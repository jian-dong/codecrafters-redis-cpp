...
TEST(SortedSetRangeTest, ZrangeReturnsSortedSetMembersByIndex) {
  ...
  ASSERT_TRUE((std::holds_alternative<RespArray>(*result))) << "ZRANGE should return a RESP array";
  ...
  ASSERT_TRUE((std::holds_alternative<RespArray>(*result))) << "ZRANGE with large stop should return a RESP array";
  ASSERT_TRUE((std::get<RespArray>(*result).values ==
  ...
  ASSERT_TRUE((std::holds_alternative<RespArray>(*result))) << "ZRANGE with a negative stop should return a RESP array";
  ASSERT_TRUE((std::get<RespArray>(*result).values ==
  ...
  ASSERT_TRUE((std::holds_alternative<RespArray>(*result))) << "ZRANGE with a negative stop offset should return a RESP array";
  ASSERT_TRUE((std::get<RespArray>(*result).values ==
             std::vector<std::string>({"baz", "caz", "paz"}))) << "ZRANGE should count negative indexes from the end";

  result = executor.Execute({"ZRANGE", "zset_key", "-2", "-1"});
  ASSERT_TRUE((result.has_value())) << "ZRANGE -2 -1 should succeed";
  ASSERT_TRUE((std::holds_alternative<RespArray>(*result))) << "ZRANGE with negative start and stop should return a RESP array";
  ASSERT_TRUE((std::get<RespArray>(*result).values ==
  ...
  ASSERT_TRUE((std::holds_alternative<RespArray>(*result))) << "ZRANGE with out-of-range negative start should return a RESP array";
  ASSERT_TRUE((std::get<RespArray>(*result).values ==
  ...
  ASSERT_TRUE((std::holds_alternative<RespArray>(*result))) << "ZRANGE past end should still return a RESP array";
  ASSERT_TRUE((std::get<RespArray>(*result).values.empty())) << "ZRANGE past end should return an empty array";
  ...
  ASSERT_TRUE((std::holds_alternative<RespArray>(*result))) << "ZRANGE with start > stop should return a RESP array";
  ASSERT_TRUE((std::get<RespArray>(*result).values.empty())) << "ZRANGE with start > stop should return an empty array";

  result = executor.Execute({"ZRANGE", "missing_key", "0", "1"});
  ASSERT_TRUE((result.has_value())) << "ZRANGE missing key should succeed";
  ASSERT_TRUE((std::holds_alternative<RespArray>(*result))) << "ZRANGE missing key should return a RESP array";
  ASSERT_TRUE((std::get<RespArray>(*result).values.empty())) << "ZRANGE missing key should return an empty array";
}
